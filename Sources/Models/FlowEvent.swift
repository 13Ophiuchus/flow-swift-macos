    ...
    struct Event: Codable, Sendable {
        ...
        public init(from decoder: Decoder) throws {
            ...
            if let txId = try? container.decodeIfPresent(Flow.ID.self, forKey: .transactionId) {
                ...
                transactionId = try container.decode(Flow.ID.self, forKey: .transactionIdCamel)
            }

            transactionIndex = try Self.decodeFlexibleInt(
                container, snakeKey: .transactionIndex, camelKey: .transactionIndexCamel
            )
            eventIndex = try Self.decodeFlexibleInt(
        ...
        public struct Result: Codable, Sendable {
            ...
            public init(from decoder: Decoder) throws {
                ...
                blockHeight = UInt64(heightString) ?? 0
...
public extension Flow.Event {
    func getField<T: Decodable>(_ name: String) -> T? {
        try? payload.fields?
            .value
            .toEvent()?
            .fields
...
public extension Flow.TransactionResult {
    func getEvent(_ type: String) -> Flow.Event? {
        events.first { $0.type == type }
    }

    func getCreatedAddress() -> String? {
