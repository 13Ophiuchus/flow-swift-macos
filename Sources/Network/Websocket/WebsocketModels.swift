...
public extension Flow {
    ...
    struct TransactionStatusBody: Decodable, Sendable {
        ...
        public init(from decoder: Decoder) throws {
            ...
            txId = try inner.decodeIfPresent(String.self, forKey: .txId)
            status = try inner.decode(Flow.Transaction.Status.self, forKey: .status)
            errorMessage = try inner.decodeIfPresent(String.self, forKey: .errorMessage)
            events = try inner.decodeIfPresent([Flow.Event].self, forKey: .events)

            if let blockIdHex = try inner.decodeIfPresent(String.self, forKey: .blockId), !blockIdHex.isEmpty {
                blockId = Flow.ID(hex: blockIdHex)
            } else {
                blockId = nil
            }

            if let computationString = try inner.decodeIfPresent(String.self, forKey: .computationUsed) {
                computationUsed = Int(computationString)
            } else {
                computationUsed = try inner.decodeIfPresent(Int.self, forKey: .computationUsed)
            }
        }
