...
public extension Flow {
    ...
    final class Publisher: @unchecked Sendable {
        ...
        public nonisolated func transactionStream() -> AsyncStream<(Flow.ID, Flow.TransactionResult)> {
        ...
        public nonisolated func accountStream() -> AsyncStream<Flow.Address> {
            AsyncStream { continuation in
                let id = UUID()
                _Concurrency.Task { @FlowWebsocketActor in
        ...
        public nonisolated func blockStream() -> AsyncStream<WSBlockHeader> {
            AsyncStream { continuation in
                let id = UUID()
                _Concurrency.Task { @FlowWebsocketActor in
        ...
        public nonisolated func walletResponseStream() -> AsyncStream<(approved: Bool, [String: Any])> {
            AsyncStream { continuation in
                let id = UUID()
        ...
        public nonisolated func errorStream() -> AsyncStream<Error> {
            AsyncStream { continuation in
                let id = UUID()
                _Concurrency.Task { @FlowWebsocketActor in
